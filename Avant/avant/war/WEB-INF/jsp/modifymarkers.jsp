...
function pintar(){
	...
	fecha_hasta = dwr.util.byId("fecha_hasta");
	if(fecha_hasta.value.replace(/^\s+|\s+$/g,'')!='')
		showhide('hasta');

	init(); // para que funcione el calendario
}
...
<fieldset>
	...
	<form:form method="post" commandName="managemarkers" cssClass="formStyle">
		...
		<tr>
			...
			<td><form:select path="tipo_marker_id" onchange="pintar();" id="select_tm">
		...
		<tr>
			...
			<td><form:input path="fecha" readonly="true" cssClass="fecha" onclick="javascript:popUpCalendar(this,this, 'dd-mm-yyyy');" />
		...
		<tr id="hasta" style="display: none;" >
			...
			<td>
				<form:input path="fecha_hasta" cssClass="fecha" id="fecha_hasta" onclick="javascript:popUpCalendar(this,this, 'dd-mm-yyyy');" />
		...
		<tr>
			...
			<td><form:select path="pelicula_id">
  	...
  	font-size: 12px;">
		...
		<tr>
			...
			<td align="right"><input id="boton" type="submit"
				value="<fmt:message key="modify"/>"></td>
		</tr>
	</table>
